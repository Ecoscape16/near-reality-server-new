...
/**
 * TOAManager handles player-specific data for Tombs of Amascut.
 * (C) Author: Savions.
 */
public class TOAManager extends AbstractTOAManager {
	...
	public static final int[] TOA_REGULAR_ITEM_IDS = {
			CrondisPuzzleEncounter.CONTAINER_ITEM_ID,
			MirrorObjectAction.MIRROR_ITEM_ID,
			ItemId.NEUTRALISING_POTION,
			ItemId.SUPPLIES,
			ItemId.HONEY_LOCUST
	};
	public static final int[] TOA_SUPPLIES_ITEM_IDS = {
			...
			ItemId.SILK_DRESSING_2, ItemId.SILK_DRESSING_1,
			ItemId.BLESSED_CRYSTAL_SCARAB_2, ItemId.BLESSED_CRYSTAL_SCARAB_1,
			ItemId.AMBROSIA_2, ItemId.AMBROSIA_1
	};
	...
	@Override
	public void clearInvocationPreset(int index) {
		updatePreset(new int[]{0, 0, 0}, index);
	}
	...
	public int[] getInvocationPreset(int index) {
		...
		return new int[]{
				player.getVarManager().getValue(baseVarId),
				player.getVarManager().getValue(baseVarId + 1),
				player.getVarManager().getValue(baseVarId + 2)
		};
	}

	@Override
	protected int getPresetBaseVarId(int index) {
		return PRESET_BASE_VAR + index * 3;
	}

	/**
	 * When a player enters the raid they will receive a package of supplies.
	 * The base package is 1 of each supply.
	 * Then, the bonus items are removed based on the active Helpful Spirit invocation:
	 *
	 * - No Helpful Spirit invocation: player receives 1 of each (base only).
	 * - "Need Some Help?" (+15): player receives 1 Liquid Adrenaline and 1 Smelling Salts, but 0 Ambrosia.
	 * - "Need Less Help?" (+25): player receives 1 Smelling Salts only.
	 * - "No Help Needed" (+40): player receives 0 of all supplies.
	 */
	private void giveEntranceItems() {
		int[] qty = getEntranceItemQuantities();
		// qty[0] = Ambrosia, qty[1] = Smelling Salts, qty[2] = Liquid Adrenaline
		if (qty[0] > 0) {
			player.getInventory().addOrDrop(new Item(ItemId.AMBROSIA_1, qty[0]));
		}
		if (qty[1] > 0) {
			player.getInventory().addOrDrop(new Item(ItemId.SMELLING_SALTS_1, qty[1]));
		}
		if (qty[2] > 0) {
			player.getInventory().addOrDrop(new Item(ItemId.LIQUID_ADRENALINE_1, qty[2]));
		}
	}

	/**
	 * Determines the entrance package quantities for each supply based on the active Helpful Spirit invocation.
	 * Returns an array: [ambrosia, smelling salts, liquid adrenaline]
	 *
	 * - If no Helpful Spirit invocation is active: returns [1, 1, 1].
	 * - For a +15 invocation ("Need Some Help?"): returns [0, 1, 1].
	 * - For a +25 invocation ("Need Less Help?"): returns [0, 1, 0].
	 * - For a +40 invocation ("No Help Needed"): returns [0, 0, 0].
	 */
	private int[] getEntranceItemQuantities() {
		// Default (no invocation): 1 of each.
		int ambrosia = 1, salts = 1, adrenaline = 1;
		for (InvocationType inv : InvocationType.VALUES) {
			if (inv.getCategory() == InvocationCategoryType.HELPFUL_SPIRIT && partySettings.isActive(inv)) {
				int mod = inv.getLevelModifier();
				if (mod == 15) { // "Need Some Help?"
					ambrosia = 0;
					// Salts and adrenaline remain 1.
				} else if (mod == 25) { // "Need Less Help?"
					ambrosia = 0;
					adrenaline = 0;
					// Only salts remain 1.
				} else if (mod == 40) { // "No Help Needed"
					ambrosia = 0;
					salts = 0;
					adrenaline = 0;
				}
				break; // Only one invocation is expected to be active.
			}
		}
		return new int[]{ambrosia, salts, adrenaline};
	}
	...
	public void enterRaid() {
		...
		if (raidParty == null && currentLobbyParty != null) {
			if (!currentLobbyParty.insideRaid()) {
				if (!currentLobbyParty.isLeader(player)) {
					player.getDialogueManager().start(new PlainChat(player, "Your leader, " +
							currentLobbyParty.getLeader().getName() + ", must enter first."));
					return;
				} else {
		...
		// Give the entrance package based on the invocation.
		giveEntranceItems();
	...
	public boolean enter(boolean checkLeader, final Object encounterType) {
		final EncounterType requestedEncounter = (EncounterType) encounterType;
		// If the requested encounter is pre-warden, bypass the fade screen and directly teleport to the wardens room.
		if (requestedEncounter.ordinal() < EncounterType.WARDENS_FIRST_ROOM.ordinal()) {
			if (checkLeader && !raidParty.isLeader(player)) {
				player.getDialogueManager().start(new PlainChat(player, "Your leader, " +
						raidParty.getLeader().getName() + ", must enter first."));
				return false;
			}
			// Construct the wardens encounter immediately.
			raidParty.constructEncounter(EncounterType.WARDENS_FIRST_ROOM);
			if (raidParty.getStartTime() == 0) {
				raidParty.setStartTime(WorldThread.getCurrentCycle());
			}
			setCurrentEncounter(EncounterType.WARDENS_FIRST_ROOM);
			Location destination = raidParty.getCurrentRaidArea().getLocation(
					EncounterType.WARDENS_FIRST_ROOM.getRandomizedSpawnTile());
		...
		// Normal flow if the requested encounter is not pre-warden.
		final EncounterType currentEncounter = raidParty.getCurrentEncounterType();
		if (!requestedEncounter.equals(currentEncounter) ||
				(raidParty.getCurrentRaidArea() != null && raidParty.getCurrentRaidArea().isDestroyed())) {
			if (checkLeader && !raidParty.isLeader(player)) {
				player.getDialogueManager().start(new PlainChat(player, "Your leader, " +
						raidParty.getLeader().getName() + ", must enter first."));
				return false;
					...
					raidParty.getPlayers().forEach(p -> {
						if (!player.getUsername().equals(p.getUsername())) {
							p.sendMessage(player.getName() +
									" has entered the Tombs of Amascut. Step inside to join " +
									(player.getAppearance().isMale() ? "him" : "her") + "...");
							p.getVarManager().sendBit(TOALobbyParty.PARTY_STATUS_VAR, 2);
						}
					});
				}
				raidParty.constructEncounter(requestedEncounter);
			}
		}
		...
		if (EncounterType.MAIN_HALL.equals(requestedEncounter)) {
		...
		WorldTasksManager.schedule(() -> {
			...
			if (raidParty.getTimeLimitMinutes() != -1 && currentEncounter == null) {
				player.sendMessage("Overall time to beat: <col=ef1020>" +
						raidParty.getTimeLimitMinutes() + ":00</col>. The timer starts upon choosing your first path.");
			}
			if (raidParty.getStartTime() == 0 && !EncounterType.MAIN_HALL.equals(requestedEncounter)) {
				raidParty.setStartTime(WorldThread.getCurrentCycle());
				if (raidParty.getTimeLimitMinutes() != -1) {
					raidParty.getPlayers().forEach(p -> p.sendMessage("Overall time to beat: <col=ef1020>" +
							raidParty.getTimeLimitMinutes() + ":00</col>. The timer has started!"));
				}
			}
			refreshTimer();
			if (EncounterType.MAIN_HALL.equals(requestedEncounter) &&
					!raidParty.getPathsCompleted().isEmpty() &&
					raidParty.getPathType() != null) {
		...
		WorldTasksManager.schedule(() -> fadeScreen.unfade(true, () -> {
			...
			if (EncounterType.MAIN_HALL.equals(requestedEncounter) &&
					!raidParty.getPathsCompleted().isEmpty() &&
					raidParty.getPathType() != null) {
				destination = raidParty.getCurrentRaidArea().getLocation(
						raidParty.getPathType().getRandomizedSpawnTile());
			} else {
				destination = raidParty.getCurrentRaidArea().getLocation(
						requestedEncounter.getRandomizedSpawnTile());
			}
			player.setLocation(destination);
		}, 0), 2);
		setCurrentEncounter(requestedEncounter);
		raidParty.getPlayers().forEach(p -> p.getTOAManager().refreshHudStates());
		...
		if (raidParty != null) {
			...
			for (int i = 0; i < TOAManager.MAX_PARTY_MEMBERS; i++) {
				if (i >= raidParty.getPlayers().size() || raidParty.getTotalTime() != -1) {
					...
					player.getVarManager().sendBit(HUD_PLAYER_SIGHT_VARBIT + i,
							p.getTemporaryAttributes().containsKey(ApmekenEncounter.SIGHT_PLAYER_ATTRIBUTE) ? 1 : 0);
					if (currentEncounterType != null &&
							!currentEncounterType.equals(p.getTOAManager().getCurrentEncounter())) {
						...
						player.getVarManager().sendBit(HUD_PLAYER_LIST_BASE_VARBIT + i, 1 +
								Math.min(28, (int) Math.floor(((double) p.getHitpoints() /
										(double) p.getMaxHitpoints()) * 28)));
					}
				}
			}
		}
	...
	public void refreshTimer() {
		if (raidParty != null && raidParty.getStartTime() > 0) {
			if (raidParty.getTotalTime() != -1) {
				...
				player.getPacketDispatcher().sendClientScript(6580,
						(int) (WorldThread.getCurrentCycle() - raidParty.getStartTime()), 0);
			}
		}
	}
	...
	public void startLeaveDialogue() {
		...
		player.getDialogueManager().start(new Dialogue(player) {
			...
			public void buildDialogue() {
				...
				options("Abandon the raid?", "Yes, abandon the raid.", "No, I want to stay.")
						.onOptionOne(() -> leaveTombs("You abandon the raid and leave the Tombs of Amascut."));
			}
		});
	}
	...
	public boolean needsAbandonRequest() {
		...
		final EncounterType currentEncounterType = raidParty.getCurrentEncounterType();
		return raidParty.getPlayers().stream()
				.anyMatch(p -> !currentEncounterType.equals(p.getTOAManager().getCurrentEncounter()));
	}
	...
	public void startAbandonDialogue(final String action, final Runnable runnable) {
		player.getDialogueManager().start(new Dialogue(player) {
			...
			public void buildDialogue() {
				...
				options(action + "?", "No, wait for any stragglers.", "Yes, abandon any stragglers.")
						.onOptionTwo(() -> {
							ImmutableList.copyOf(raidParty.getPlayers()).stream()
									.filter(p -> !raidParty.getCurrentRaidArea().getPlayers().contains(p))
									.forEach(p -> {
	...
	@Override
	public void initialize(AbstractTOAManager manager) {
	...
	public void onLogin() {
		if (toaPlayerLogoutState != null) {
			...
			if (regionArea instanceof final TOARaidArea area &&
					area.getParty().getOriginalPlayers().contains(player.getUsername())) {
				if (area.hasFailedTombs()) {
					...
					player.setLocation(area.getLocation(
							EncounterType.fromBase(toaPlayerLogoutState.getEncounterType()).getRandomizedSpawnTile()));
					setCurrentEncounter(area.getParty().getCurrentEncounterType());
					if (toaPlayerLogoutState.isDuringChallenge()) {
						player.sendMessage("You logged out during the challenge. Total deaths: <col=ff0000>" +
								area.getParty().getTotalDeaths() + "</col>.");
		...
		if (player.getArea() instanceof TOARaidArea raidArea) {
			...
			final Runnable runnable = () -> {
				...
				if (!raidParty.getCurrentEncounterType().equals(current)) {
					raidParty.getPlayers().stream()
							.filter(p -> !player.getUsername().equals(p.getUsername()))
							.forEach(p -> p.sendMessage(player.getUsername() + " has proceeded to the next challenge. Join " +
									(player.getAppearance().isMale() ? "him" : "her") + "..."));
				}
			};
			if (!quickUse) {
				player.getDialogueManager().start(new OptionDialogue(player, "Are you ready to proceed?",
						new String[]{"Yes.", "No."}, new Runnable[]{runnable, null}));
			} else {
				runnable.run();
			}
		}
	...
	public void reSupply() {
		...
		itemLoop:
		for (Item item : suppliesContainer.getItemsAsList()) {
			if (item == null) {
				continue;
			}
			for (int id : TOA_SUPPLIES_ITEM_IDS) {
	...
	public void setPartySettings(Object partySettings) {
		this.partySettings = (TOAPartySettings) partySettings;
	}

	@Override
	public Object getToaPlayerLogoutState() {
		return toaPlayerLogoutState;
	}

	@Override
	public void setToaPlayerLogoutState(Object toaPlayerLogoutState) {
		this.toaPlayerLogoutState = (TOAPlayerLogoutState) toaPlayerLogoutState;
	}

	@Override
	public EncounterType getCurrentEncounter() {
	...
	@Override
	public void setIndividualDeaths(int individualDeaths) {
		this.individualDeaths = individualDeaths;
	}
	...
	@Override
	public void setRewardContainer(Container rewardContainer) {
