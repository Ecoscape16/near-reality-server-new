...
public abstract class TOARaidArea extends AbstractTOARaidArea implements HitProcessPlugin, CannonRestrictionPlugin, LogoutPlugin, LayableObjectPlugin, MusicPlugin, DeathPlugin, TeleportPlugin, EdiblePlugin, DrinkablePlugin, DropPlugin {
	...
	public void leave(Player player, boolean logout) {
		...
		if (logout) {
			...
			// Instead of calling TOAManager.getRandomizedOutsideLocation(), we use OUTSIDE_LOCATION.transform(...)
			player.setLocation(OUTSIDE_LOCATION.transform(Utils.random(2), Utils.random(1)));
		}
	...
	public void onLogout(@NotNull Player player) {
		...
		player.getTOAManager().setToaPlayerLogoutState(new TOAPlayerLogoutState(
				encounterType.toBase(),
				EncounterStage.STARTED.equals(stage) && insideChallengeArea(player),
				die ? party.getCurrentRaidArea().getLocation(encounterType.getRandomizedSpawnTile()) : player.getLocation(),
				party.getPermittedTeamDeaths() == -1));
		ConsumableEffects.resetSalt(player);
		if (die) {
		...
		// Fixed call: use OUTSIDE_LOCATION.transform(...) instead of TOAManager.getRandomizedOutsideLocation()
		player.setLocation(OUTSIDE_LOCATION.transform(Utils.random(2), Utils.random(1)));
	}
	...
	@Override
	public boolean hit(Player source, Entity target, Hit hit, float modifier) {
	...
	@Override
	public boolean eat(Player player, Edible food) {
		if (onDiet) {
	...
	public int getStartTeamSize() {
		return teamSize;
	}

	public boolean hasFailedTombs() {
		return tombsFailure;
	}
