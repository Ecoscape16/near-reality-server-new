...
import com.near_reality.game.content.dt2.drops.hasReceivedFrozenTablet
import com.near_reality.game.content.dt2.drops.hasReceivedIceQuartz
...
class WhispererDropTable : NPCDropTableScript() {
    init {
        npcs(THE_WHISPERER)

        onDeath {
            val whisperer = this.npc
            val baseRate = 1

            if (baseRate > 0) {
                if (Utils.random(99) < baseRate) {
                    rollTable(killer, DropTableType.Standalone, WhispererUniques.staticTable).forEach {
                        npc.dropItemAtKiller(killer, Item(it.id, 1))
                    }
                    rollStaticTableAndDrop(killer, DropTableType.Tertiary)
                    ...
                    rollStaticTableAndDropBelowPlayer(killer, DropTableType.Main)
                }
            }
        }
            ...
            appendDrop(DisplayedDrop(BELLATOR_VESTIGE, 1, 1, 1.0))
            appendDrop(DisplayedDrop(SIRENS_STAFF, 1, 1, 800.00))
        ...
        object WhispererUniques : StandaloneDropTableBuilder({
            ...
            static {
                ...
                BELLATOR_VESTIGE quantity 1 rarity 3
                SIRENS_STAFF quantity 1 rarity 2
