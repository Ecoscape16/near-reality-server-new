...
public class AlchemicalHydraProcessor extends DropProcessor {
    ...
    @Override
    public void attach() {
        appendDrop(new DisplayedDrop(ItemId.DRAGON_THROWNAXE, 500, 1000, 60));
        appendDrop(new DisplayedDrop(ItemId.DRAGON_KNIFE, 500, 1000, 60));
    ...
    public void onDeath(final NPC npc, final Player killer) {
        ...
        if(randomRoll(killer, 59) == 0) {
            npc.dropItem(killer, new Item(ItemId.DRAGON_THROWNAXE));
            return;
        }
        if(randomRoll(killer, 59) == 0) {
            npc.dropItem(killer, new Item(ItemId.DRAGON_KNIFE));
            return;
        }
        ...
        if(randomRoll(killer, 49) == 0) {
            npc.dropItem(killer, new Item(ItemId.HYDRAS_HEART));
            return;
        }
        if(randomRoll(killer, 49) == 0) {
            npc.dropItem(killer, new Item(ItemId.HYDRAS_FANG));
            return;
        }
