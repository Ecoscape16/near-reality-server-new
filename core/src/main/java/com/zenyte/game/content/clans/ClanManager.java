...
public final class ClanManager implements ScheduledExternalizable {
    ...
    private static int canTalk(@NotNull final Player player, @NotNull final Player clanOwner,
                                   @NotNull final ClanChannel channel) {
        if(!player.isStaff() && (player.getAttributes().get("cc_last_talk") != null) && (long) player.getAttributes().get("cc_last_talk") >= (System.currentTimeMillis() - (player.getMemberRank().equalToOrGreaterThan(MemberRank.Emerald) ? TimeUnit.TICKS.toMillis(2) : TimeUnit.TICKS.toMillis(5)))) {
