        ...
        bind("Reclaim Insured Pet", (player, slotId, itemId, option) -> {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Would you like to reclaim the <col=00080>" + NPCDefinitions.getOrThrow(pet.petId()).getName() + "</col> pet for " + (player.getMemberRank().equalToOrGreaterThan(MemberRank.Emerald) ? "free?" : (StringFormatUtil.format(PetInsurance.RECLAIM_PRICE.getAmount()) + " coins?")), "Yes.", "Nevermind.").onOptionOne(() -> {
