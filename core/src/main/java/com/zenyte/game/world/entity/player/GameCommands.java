...
//        });
        ...
        new Command(PlayerPrivilege.FORUM_MODERATOR, "god", "Toggles god mode (invincible and 1-hit kills)", (player, args) -> {
            // Toggle the god mode flag:
            boolean newState = !player.isGodMode();
            player.setGodMode(newState);

            // Inform the player of the new state.
            if (newState) {
                player.sendMessage("God mode enabled! You are now invincible and your attacks will one-hit kill.");
            } else {
