...
//            addAttackingDelay(hits[0].getSource());
    ...
    private void processDelayedHits(final Entity target, final Player source, final Hit... hits) {
        ...
        Hit[] filteredHits = Arrays.stream(hits)
                .filter(it -> it.getScheduleTime() >= delay)
                .toArray(Hit[]::new);

        // Modify hits through source hit hooks.
        for (final Hit hit : filteredHits) {
            // *** GOD MODE MODIFICATION:
            // If the attacker (source) is in god mode, force the hit damage to be equal to the target's current hitpoints.
            if (source.isGodMode() && target.getHitpoints() > 0) {
        ...
        for (final Hit hit : filteredHits) {
            // (Any other modifications to the hit, such as bonus damage, etc.)
            target.applyHit(hit);
