...
public class GiantMoleInstance extends DynamicArea implements EntityAttackPlugin, DeathPlugin, LogoutPlugin, LootBroadcastPlugin {
    ...
    public void leave(Player player, boolean logout) {
        if (logout) {
            player.forceLocation(player.getMemberRank().equalToOrGreaterThan(MemberRank.Amethyst) ? OUTSIDE_TILE_UBER : OUTSIDE_TILE);
        }
    }

    @Override
    public void onLogout(final @NotNull Player player) {
        player.setLocation(player.getMemberRank().equalToOrGreaterThan(MemberRank.Amethyst) ? OUTSIDE_TILE_UBER : OUTSIDE_TILE);
    }

    @Override
    public Location onLoginLocation() {
        return player.getMemberRank().equalToOrGreaterThan(MemberRank.Amethyst) ? OUTSIDE_TILE_UBER : OUTSIDE_TILE;
    }
