...
public class Zahur extends NPCPlugin {
    ...
    public static final void crushSecondaries(@NotNull final Player player, @NotNull final NPC npc) {
        ...
        final boolean free = player.getMemberRank().equalToOrGreaterThan(MemberRank.Onyx);
        if (cost == -1) {
            return;
        }
        player.getDialogueManager().start(new Dialogue(player, npc) {
    ...
    private void makeUnfinishedPotions(final Player player, final NPC npc) {
        ...
        int amount = player.getMemberRank().equalToOrGreaterThan(MemberRank.Onyx) ? Integer.MAX_VALUE : price == 0 ? vialsOfWater : Math.min(coins / price, vialsOfWater);
    ...
    private int getPrice(final Player player) {
        if (player.getMemberRank().equalToOrGreaterThan(MemberRank.Onyx)) {
            ...
            return 100;
        } else if (player.getMemberRank().equalToOrGreaterThan(MemberRank.Pyrite)) {
            return 125;
